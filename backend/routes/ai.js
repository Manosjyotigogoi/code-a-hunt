...
async function callAI(systemPrompt, userPrompt, maxTokens = 1000) {
  ...
  if (!HF_API_KEY) throw new Error('Missing Hugging Face API key. Set HF_API_KEY.');
  ...
  for (const model of modelCandidates) {
    ...
    const legacyResponse = await fetch(legacyEndpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${HF_API_KEY}` },
      body: JSON.stringify({ inputs: `${systemPrompt}\n\n${userPrompt}`, parameters: { max_new_tokens: maxTokens, return_full_text: false } }),
    });
    ...
    const generated = Array.isArray(legacyData) ? legacyData?.[0]?.generated_text : legacyData?.generated_text;
...
// POST /api/ai/grade  ← NEW endpoint for grading short/long answers
router.post('/grade', authMiddleware, async (req, res) => {
  ...
  try {
    const system = `You are a strict but fair examiner. Grade each student answer compared to the model answer.
Return ONLY a JSON array with one object per answer: { "score": 0-10, "feedback": "one sentence feedback", "isCorrect": true/false }
Be consistent: full marks for complete correct answers, partial marks for partially correct, 0 for wrong/blank.`;

    const prompt = answers.map((a, i) =>
      `Q${i + 1} [${a.type}]: ${a.question}\nModel Answer: ${a.correctAnswer}\nStudent Answer: ${a.userAnswer || '(no answer)'}`
    ).join('\n\n');

    const raw = await callAI(system, prompt, 800);
    const grades = parseJSON(raw) || answers.map(() => ({ score: 0, feedback: 'Could not grade.', isCorrect: false }));
    res.json({ grades });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/growth', authMiddleware, async (req, res) => {
  ...
  if (!subject) return res.status(400).json({ error: 'subject is required.' });
  try {
    const system = `You are a learning analyst. Analyze student performance data and provide insights.
Return JSON: { "strengths": [], "weaknesses": [], "suggestions": [], "overallScore": 0-100, "trend": "improving/declining/stable" }
All scores are already normalized to 0-100 percentage scale so they are comparable regardless of question count.`;
    const prompt = `Subject: ${subject}\nQuiz attempts (all scores are percentages 0-100): ${JSON.stringify(scores || [])}\nTopics covered: ${(topics || []).join(', ')}`;
    const raw = await callAI(system, prompt, 800);
